...
from agents.prescriptive_optimization_agent import recommend_action, make_free_recommendation
...
from agents.decision_logger import get_history, log_recommendation
...
def _init_observability():
    """Initialize Langfuse once per app process (survives reruns)."""
    return observability.warm_up()

_OBS_STATUS = _init_observability()

from agents.fault_classifier import classify_fault, FAULT_DESCRIPTIONS, guess_unknown_fault
from agents.uncertainty_detector import apply_safe_defaults, detect_uncertainty
...
section[data-testid="stSidebar"] {
    ...
    border-right: 1px solid #D2DAEA !important;
}
...
section[data-testid="stSidebar"] [data-baseweb="select"] > div {
    background-color: #FFFFFF !important;
    border: 1px solid #C7D0E4 !important;
...
.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #2D5ECC 0%, #2d5ecc 100%) !important;
...
.stTabs [aria-selected="true"] {
    color: #2D5ECC !important;
    border-bottom-color: #2D5ECC !important;
    background-color: transparent !important;
}
...
/* ── HR dividers ── */
hr { border-color: #D2DAEA !important; }

/* ── Markdown text color ── */
[data-testid="stMarkdownContainer"] p { color: #4B5563; }
...
[data-testid="stPopover"] [data-testid="stVerticalBlockBorderWrapper"] {
    ...
    background: linear-gradient(160deg, rgba(255,255,255,0.92) 0%, rgba(244,246,251,0.95) 100%) !important;
    backdrop-filter: blur(16px) !important;
    -webkit-backdrop-filter: blur(16px) !important;
    border: 1px solid rgba(45,94,204,0.25) !important;
    box-shadow: 0 8px 40px rgba(45,94,204,0.15) !important;
}
...
.chat-header {
    ...
    border-bottom: 1px solid rgba(45,94,204,0.2);
    margin-bottom: 4px;
}
.chat-header-title {
...
[data-testid="stPopover"] div strong { font-weight: 700 !important; color: #2D5ECC !important; }
[data-testid="stPopover"] div code {
    background: rgba(45,94,204,0.10); padding: 1px 6px; border-radius: 5px;
    font-size: 0.82em; font-family: 'JetBrains Mono', monospace; color: #6D28D9;
}
...
/* ── Source chip (light tint) ── */
/* note: source line is inline-styled; this nudges its accent toward blue */
</style>
""")
        ...
        with _hbox:
            ...
            for _idx, _m in enumerate(_ch):
                if _m["role"] == "user":
                    ...
                    _bubble_style = (
                        "background:linear-gradient(135deg,rgba(45,94,204,0.10) 0%,rgba(124,92,246,0.08) 100%);"
                        "color:#1A1D2E;padding:11px 15px;border-radius:16px 16px 16px 5px;"
                        "margin:7px auto 7px 0;max-width:82%;width:fit-content;"
                        "font-size:0.86rem;line-height:1.55;"
                        "backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);"
                        "border:1px solid rgba(45,94,204,0.30);"
                        "box-shadow:0 4px 22px rgba(45,94,204,0.12);"
                    )
                st.markdown(
                ...
                if _m.get("role") == "assistant":
                    ...
                    if _srcs:
                        ...
                        st.markdown(
                            f'<div style="font-size:0.7rem;color:#4B5563;'
                            f'font-family:JetBrains Mono,monospace;margin:-2px 0 10px 4px;'
                            f'padding:3px 8px;background:rgba(45,94,204,0.08);'
                            f'border-left:2px solid #2D5ECC;border-radius:0 6px 6px 0;'
...
def _urgency_color(u: str) -> str:
    return {"monitor": "#0E9F6E", "planned": "#2D5ECC",
            "urgent": "#B45309", "emergency": "#DC2626"}.get(u, "#6B7280")
...
def _approval_color(a: str) -> str:
    return {"pending": "#2D5ECC", "escalated": "#DC2626",
            "approved": "#0E9F6E", "rejected": "#DC2626"}.get(a, "#6B7280")


_CARD = "background:#EAEEF7;border-radius:12px;border:1px solid #D2DAEA;padding:1.8rem 2rem;min-height:10rem;box-sizing:border-box;"
_LBL  = "color:#6B7280;font-family:'JetBrains Mono',monospace;font-size:0.7rem;letter-spacing:0.12em;text-transform:uppercase;margin-bottom:0.5rem;"


def _detail_card(label: str, value: str, sub: str = "",
                 value_color: str = "#1A1D2E", sub_color: str = "#6B7280",
    ...
    cap_html = (
        f'<div style="color:#94A0B8;font-family:\'JetBrains Mono\',monospace;'
...
def _format_rationale(text: str) -> str:
    ...
    """
    _HEAD = (
        'display:block;color:#1A1D2E;font-weight:700;'
...
def _th(text: str) -> str:
    return (
        f'<th style="padding:0.65rem 1rem;text-align:left;color:#6B7280;'
...
def _td(text: str, mono: bool = False) -> str:
    ...
    return (
        f'<td style="padding:0.7rem 1rem;color:#4B5563;font-size:0.85rem;{ff}">{text}</td>'
    )
...
<div style="padding:0.4rem 0 0.2rem 0;">
  <div style="font-size:1.02rem;font-weight:700;color:#1A1D2E;letter-spacing:0.02em;">Prescriptive Optimization Agent</div>
  <div style="font-size:0.78rem;color:#6B7280;margin-top:2px;">Downtime Response Orchestrator</div>
</div>
""", unsafe_allow_html=True)
...
if input_mode == "Demo scenario":
    st.sidebar.markdown(
        '<div style="font-size:0.68rem;color:#6B7280;letter-spacing:0.1em;'
...
def _prov_row(k: str, v: str) -> str:
    ...
    return (
        ...
        f'gap:0.5rem;padding:0.16rem 0;flex-wrap:wrap;">'
        f'<span style="color:#6B7280;font-size:0.7rem;">{k}</span>'
        f'<span style="color:#1A1D2E;font-family:\'JetBrains Mono\',monospace;'
        f'font-size:0.7rem;font-weight:500;text-align:right;'
        f'overflow-wrap:anywhere;word-break:break-word;min-width:0;">{v}</span></div>'
    )
...
def _prov_group(agent_label: str, color: str, rows: str) -> str:
    ...
    return (
        f'<div style="background:#EAEEF7;'
        f'border:1px solid #D2DAEA;border-left:3px solid {color};border-radius:10px;'
...
if input_mode == "Demo scenario":
    ...
    if selected_label in SCENARIO_MAP:
        ...
        if _desc:
            st.sidebar.markdown(
                f'<div style="background:linear-gradient(135deg,#F5F0FF,#EDE4FB);'
                f'border:1px solid #E9D5FF;border-left:3px solid #A100FF;border-radius:12px;'
                ...
                f'<span style="color:#6D28D9;font-family:\'JetBrains Mono\',monospace;font-size:0.6rem;'
                f'letter-spacing:0.1em;text-transform:uppercase;font-weight:600;">What this scenario is</span>'
                f'</div>'
                f'<div style="color:#1A1D2E;font-size:0.78rem;line-height:1.55;">{_desc}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
    ...
    if selected_label in SCENARIO_MAP:
        ...
        _grp_63 = _prov_group(
            "Fault Diagnosis · Agent 6.3", "#2D5ECC",
        ...
        st.sidebar.markdown(
            ...
            f'<span style="color:#6B7280;font-family:\'JetBrains Mono\',monospace;font-size:0.62rem;'
    ...
    st.sidebar.markdown(
        '<div style="font-size:0.72rem;color:#6B7280;line-height:1.5;margin-bottom:0.6rem;">'
    ...
    if st.session_state.get("awaiting_confirmation"):
        ...
        if _pending_cls:
            if _pending_cls.get("fault_mode") == "unknown":
                ...
                _mode_pretty = _mode_raw.replace("_", " ").title()
                _desc = FAULT_DESCRIPTIONS.get(_mode_raw, "")
                _msg = f"This looks like a **{_mode_pretty}**."
                if _desc:
                    _msg += f"\n\n{_desc}"
                _msg += "\n\nDoes this match? Proceed, or rephrase your description."
...
with st.sidebar.expander("\U0001F4CB  View all scenarios", expanded=False):
    ...
    for _name, _desc in _SCENARIO_CATALOG:
        _cat_rows += (
            ...
            f'<div style="color:#2D5ECC;font-weight:600;font-size:0.78rem;'
            f'margin-bottom:0.2rem;">{_name}</div>'
            f'<div style="color:#6B7280;font-size:0.72rem;line-height:1.45;">{_desc}</div>'
            f'</div>'
        )
...
def _persona_row(role_name: str, note: str, accent: str) -> str:
    ...
    return (
        ...
        f'<div style="color:#6B7280;font-size:0.7rem;line-height:1.4;">{note}</div>'
        f'</div>'
    )
...
def _group_heading(text: str) -> str:
    return (
        f'<div style="color:#6B7280;font-family:\'JetBrains Mono\',monospace;font-size:0.62rem;'
...
with st.sidebar.expander("\U0001F465  View all personas", expanded=False):
    ...
    _html = (
        '<div style="color:#6B7280;font-size:0.68rem;line-height:1.45;margin-bottom:0.4rem;">'
    ...
    for _tier in ("normal", "escalated", "executive"):
        ...
        _html += _persona_row(_label, _TIER_WHEN.get(_tier, _tier), "#2D5ECC")
    ...
    for _c in _contributors.values():
        ...
        _html += _persona_row(_label, _concern, "#0E9F6E")
...
st.sidebar.markdown("---")
st.sidebar.markdown(
    '<div style="font-size:0.68rem;color:#D2DAEA;text-align:center;line-height:1.7;">'
...
for _i, _name in enumerate(_PIPELINE):
    _divider = "border-right:1px solid #D2DAEA;" if _i < len(_PIPELINE) - 1 else ""
    if _i == _MINE_IDX:
        _segs += (
            ...
            f'background:linear-gradient(135deg,#EDE4FB,#F3ECFD);'
            f'box-shadow:inset 0 0 12px rgba(161,0,255,0.18);">'
            ...
            f'<span style="color:#6D28D9;font-weight:700;font-size:0.64rem;'
        ...
        _segs += (
            ...
            f'<span style="color:#6B7280;font-size:0.64rem;">{_name}</span></div>'
        )

st.markdown(
    f'<div style="display:flex;align-items:stretch;width:100%;background:#EEF1F8;'
    f'border:1px solid #D2DAEA;border-radius:8px;overflow:hidden;margin:0.1rem 0 0.2rem 0;">'
    f'{_segs}</div>'
    f'<div style="color:#6B7280;font-size:0.58rem;font-family:\'JetBrains Mono\',monospace;'
...
# ── Page header ───────────────────────────────────────────────────────────────
st.markdown("""
<div style="padding:0.25rem 0 1.5rem 0;border-bottom:1px solid #D2DAEA;margin-bottom:1.75rem;">
  <div style="font-size:0.68rem;font-weight:500;color:#2D5ECC;letter-spacing:0.15em;
          text-transform:uppercase;margin-bottom:0.4rem;">
Downtime Response Orchestrator &nbsp;&middot;&nbsp; Agent 6.6
  </div>
  <div style="font-size:1.95rem;font-weight:700;color:#1A1D2E;line-height:1.15;margin-bottom:0.35rem;">
Prescriptive Maintenance Recommendation
  </div>
  <div style="font-size:0.87rem;color:#6B7280;max-width:720px;">
...
if not st.session_state.get("rec"):
    st.markdown("""
<div style="background:#EAEEF7;border-radius:14px;border:1px solid #D2DAEA;
            padding:3rem 2rem;text-align:center;margin-top:0.5rem;">
  <div style="font-size:2.2rem;color:#2D5ECC;margin-bottom:0.8rem;">&#9881;</div>
  ...
  <div style="font-size:0.85rem;color:#6B7280;">
    ...
    <strong style="color:#2D5ECC;">Generate Recommendation</strong>.
  </div>
</div>
""", unsafe_allow_html=True)
    st.stop()
...
if is_llm:
    st.markdown(
        '<div style="background:linear-gradient(135deg,#FEF6E7,#FFFDF7);'
        'border:1px solid #B4530955;border-left:4px solid #B45309;border-radius:12px;'
        'padding:1.1rem 1.4rem;margin-bottom:1.25rem;box-shadow:0 2px 12px rgba(255,183,64,0.12);">'
        '<div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.4rem;">'
        '<span style="font-size:1.15rem;">&#9888;</span>'
        '<span style="color:#B45309;font-weight:700;font-size:1rem;'
        'font-family:\'Space Grotesk\',sans-serif;">AI-Generated Suggestion — Novel Scenario</span>'
        '</div>'
        '<div style="color:#4B5563;font-size:0.86rem;line-height:1.6;">'
...
if rec.recommendation_status == "blocked_invalid_input":
    ...
    _BOX_ERR = (
        "background:#EEF1F8;border-radius:8px;border-left:3px solid #e85d4a;"
        "padding:0.6rem 0.9rem;color:#DC2626;font-size:0.85rem;"
        "font-family:'JetBrains Mono',monospace;"
    )
    _BOX_MSG = (
        "background:#EEF1F8;border-radius:8px;border-left:3px solid #2D5ECC;"
        "padding:0.6rem 0.9rem;color:#4B5563;font-size:0.85rem;"
    )
    _err_rows = "".join(
    ...
    st.markdown(
        f'<div style="background:#EAEEF7;border-radius:14px;border:1px solid #D2DAEA;'
        ...
        f'<div style="color:#1A1D2E;font-size:0.88rem;font-weight:600;margin-bottom:0.6rem;">'
...
st.markdown(
    ...
    f'<span style="color:#94A0B8;font-family:\'JetBrains Mono\',monospace;'
...
if _cls and not st.session_state.get("awaiting_confirmation"):
    _msg = (f"🔍 Interpreted your description as "
            f"**{_cls['fault_mode'].replace('_', ' ').title()}**.")
    if _cls.get("alternatives"):
        _alts_pretty = ', '.join(a.replace('_', ' ').title() for a in _cls['alternatives'])
        _msg += f" (Other possibilities considered: {_alts_pretty}.)"
    st.info(_msg)
...
if _unc and _unc.is_partial and _rec_exists and _is_partial_scenario:
    st.markdown(
        '<div style="background:#B4530918;border:1px solid #B4530944;'
        'border-left:4px solid #B45309;border-radius:8px;'
        'padding:0.75rem 1.25rem;margin-bottom:1rem;">'
        '<span style="color:#B45309;font-weight:700;font-size:0.85rem;'
        'letter-spacing:0.05em;">&#9888; PARTIAL DATA</span>'
        '<span style="color:#4B5563;font-size:0.82rem;margin-left:8px;">'
    ...
    with st.expander("View uncertainty details", expanded=False):
        for i, flag in enumerate(_unc.flags, 1):
            st.markdown(
                f'<div style="background:#EAEEF7;border-left:3px solid '
                f'#B45309;padding:0.6rem 1rem;margin-bottom:0.5rem;'
                f'border-radius:0 6px 6px 0;font-size:0.83rem;'
                f'color:#4B5563;">'
                f'<strong style="color:#B45309;">{i}.</strong> {flag}'
                f'</div>',
                unsafe_allow_html=True,
            )
        st.caption(
...
st.markdown(
    ...
    f'<span style="font-size:1.75rem;font-weight:700;color:#1A1D2E;">{action_display}</span>'
    f'{_pill(rec.urgency.upper(), urg_color)}'
    f'{_ai_tag}'
    f'</div>'
    f'<div style="color:#4B5563;font-size:0.9rem;line-height:1.6;margin-bottom:0.35rem;">{desc_text}</div>'
    ...
    f'<span style="color:#6B7280;">ASSET&nbsp;</span>'
    f'<span style="color:#1A1D2E;">{rec.asset_id}</span></span>'
    f'<span style="font-family:\'JetBrains Mono\',monospace;font-size:0.78rem;">'
    f'<span style="color:#6B7280;">BEARING&nbsp;</span>'
    f'<span style="color:#1A1D2E;">{rec.bearing_id}</span></span>'
    f'<span style="font-family:\'JetBrains Mono\',monospace;font-size:0.78rem;">'
    f'<span style="color:#6B7280;">DURATION&nbsp;</span>'
    f'<span style="color:#1A1D2E;">{dur_display}</span></span>'
    f'</div>'
    f'</div>',
    unsafe_allow_html=True,
)
...
st.markdown(
    f'<div style="background:#EAEEF7;border-radius:14px;border:1px solid #D2DAEA;'
    f'border-left:4px solid #2D5ECC;padding:1.6rem 2rem;margin-bottom:1.5rem;">'
    f'<div style="{_LBL}">Why This Recommendation</div>'
    f'<div style="color:#4B5563;font-size:0.9rem;line-height:1.8;">'
...
if rec.recommendation_status == "blocked_no_part":
    ...
    _BOX_AMB = (
        "background:#EEF1F8;border-radius:8px;border-left:3px solid #e8a84a;"
        "padding:0.6rem 0.9rem;color:#4B5563;font-size:0.85rem;"
    )
    _BOX_RED = (
        "background:#EEF1F8;border-radius:8px;border-left:3px solid #e85d4a;"
    ...
    st.markdown(
        ...
        f'<div style="color:#4B5563;font-size:0.87rem;line-height:1.7;margin-bottom:0.8rem;">'
        f'<div>Intended action: <span style="color:#1A1D2E;font-weight:600;">'
        f'{rec.recommended_action.name}</span></div>'
        f'<div>Blocked part: <span style="color:#1A1D2E;font-family:\'JetBrains Mono\','
        f'monospace;">{_bp_part}</span></div>'
        f'<div>Part lead time: <span style="color:#1A1D2E;font-family:\'JetBrains Mono\','
        f'monospace;">{_bp_lead}d</span></div>'
        f'<div style="color:#6B7280;font-size:0.8rem;margin-top:0.4rem;">{_bp_ev_part}</div>'
        f'</div>'
        f'<div style="display:flex;flex-direction:column;gap:0.55rem;margin-bottom:0.85rem;">'
        f'<div style="{_BOX_AMB}">Procure: <span style="color:#1A1D2E;font-family:\'JetBrains Mono\','
        ...
        f'<div style="{_BOX_AMB}">Escalate: <span style="color:#1A1D2E;">'
...
if (
    ...
    if _nw_windows:
        ...
        _opt2_text = (
            f'Option 2 — Wait for <span style="color:#1A1D2E;font-family:\'JetBrains Mono\','
    ...
    st.markdown(
        ...
        f'<div style="color:#4B5563;font-size:0.87rem;line-height:1.7;margin-bottom:0.9rem;">'
        f'No planned window fits within the RUL — '
        f'<span style="color:#6B7280;font-size:0.82rem;">{_rul_ev}</span>'
        f'</div>'
        f'<div style="display:flex;flex-direction:column;gap:0.55rem;margin-bottom:0.85rem;">'
        f'<div style="background:#EEF1F8;border-radius:8px;border-left:3px solid #e85d4a;'
        f'padding:0.6rem 0.9rem;color:#4B5563;font-size:0.85rem;">'
        f'Option 1 — <b>Act now</b>: higher operational disruption, lower asset risk</div>'
        f'<div style="background:#EEF1F8;border-radius:8px;border-left:3px solid #e8a84a;'
        f'padding:0.6rem 0.9rem;color:#4B5563;font-size:0.85rem;">'
        f'{_opt2_text}</div>'
        f'</div>'
        f'<div style="color:#6B7280;font-size:0.82rem;font-style:italic;">'
...
with tab_parts:
    if is_llm:
        if rec.required_parts:
            ...
            st.markdown(
                f'<div style="padding:12px 16px;border:1px solid #B4530955;border-radius:10px;'
                f'background:#B4530911;color:#f5d78a;font-size:0.85rem;margin-bottom:0.75rem;">'
            ...
            for p in rec.required_parts:
                _llm_rows += (
                    f'<tr style="border-bottom:1px solid #D2DAEA;">'
                    + _td(p.part_number, mono=True)
                    + _td(str(p.quantity))
                    + _td(str(p.lead_time_days))
                    + f'<td style="padding:0.65rem 1rem;">'
                    + _pill("In inventory — verify", "#B45309")
                    + f'</td></tr>'
                )
            st.markdown(
                f'<div style="background:#EAEEF7;border-radius:12px;border:1px solid #D2DAEA;overflow:hidden;">'
                f'<table style="width:100%;border-collapse:collapse;">'
                f'<thead><tr style="background:#EEF1F8;border-bottom:1px solid #D2DAEA;">'
            ...
            st.markdown(
                f'<div style="{_CARD}color:#6B7280;font-size:0.9rem;">'
        ...
        for p in rec.required_parts:
            if p.lead_time_days == 0:
                status_html = _pill("In stock", "#0E9F6E")
                lead_display = str(p.lead_time_days)
            else:
                status_html = _pill(f"Order — {p.lead_time_days}d lead time", "#B45309")
                lead_display = str(p.lead_time_days)
            rows_html += (
                f'<tr style="border-bottom:1px solid #D2DAEA;">'
                + _td(p.part_number, mono=True)
        ...
        st.markdown(
            f'<div style="{_CARD}color:#6B7280;font-size:0.9rem;">No parts required for this action.</div>',
            unsafe_allow_html=True,
        )
    if not is_llm:
...
with tab_schedule:
    if rec.window_chosen:
        ...
        if win:
            ...
            if win.get("notes"):
                st.markdown(
                    f'<div style="background:#EEF1F8;border-radius:8px;border:1px solid #D2DAEA;'
                    f'padding:0.85rem 1.2rem;margin-top:1rem;color:#6B7280;font-size:0.85rem;">'
                    f'{win["notes"]}</div>',
                    unsafe_allow_html=True,
                )
        else:
            st.markdown(
                f'<div style="{_CARD}color:#4B5563;font-size:0.9rem;">'
                f'Window <span style="font-family:\'JetBrains Mono\',monospace;color:#1A1D2E;">'
        ...
        st.markdown(
            f'<div style="background:#FFFBEB;border-radius:12px;border:1px solid #DC262644;'
            f'padding:1.3rem 1.6rem;color:#B45309;font-size:0.9rem;">'
...
with tab_evidence:
    ...
    evidence_rows = [
        ...
        ("Guidance (SOP)",       "Agent 6.5 — Knowledge Agent",                      ev.get("guidance", "—")),
    ]
    ...
    for _claim, _value, _source in evidence_rows:
        ev_rows_html += (
            f'<tr style="border-bottom:1px solid #D2DAEA;">'
            f'<td style="padding:0.7rem 1rem;color:#1A1D2E;font-weight:600;'
            f'font-size:0.87rem;white-space:nowrap;">{_claim}</td>'
            f'<td style="padding:0.7rem 1rem;color:#4B5563;'
            f'font-family:\'JetBrains Mono\',monospace;font-size:0.82rem;">{_value}</td>'
            f'<td style="padding:0.7rem 1rem;color:#94A0B8;'
...
if _contributors:
    ...
    for _c in _contributors:
        ...
        cards_html += (
            f'<div style="background:#EEF1F8;border-radius:10px;border:1px solid #D2DAEA;'
            f'border-left:3px solid #2D5ECC;padding:0.9rem 1.1rem;flex:1 1 230px;min-width:230px;">'
            f'<div style="color:#1A1D2E;font-weight:600;font-size:0.9rem;margin-bottom:0.25rem;'
            f'overflow-wrap:break-word;">{_title}</div>'
            f'<div style="color:#6B7280;font-size:0.8rem;line-height:1.5;'
    ...
    st.markdown(
        ...
        f'<div style="color:#6B7280;font-size:0.8rem;margin-bottom:1rem;">'
...
st.markdown(
    ...
    f'<span style="font-size:1.1rem;font-weight:600;color:#1A1D2E;">{person}</span>'
    f'{_pill(current_approval.upper(), appr_color)}'
    f'</div>'
    f'<div style="color:{appr_color};font-size:0.92rem;font-weight:600;margin-bottom:0.35rem;">'
    f'{_appr_phrase}</div>'
    f'<div style="color:#94A0B8;font-family:\'JetBrains Mono\',monospace;font-size:0.72rem;">'
...
st.markdown(
    ...
    f'<div style="color:#6B7280;font-size:0.8rem;margin-bottom:0.8rem;">'
