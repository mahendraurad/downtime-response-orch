...
def resolve_responsible_person(
    ...
    """
    Look up who is responsible for approving this recommendation.
    Returns a (role_name, persona_id) tuple.

    Rule 1 — escalation first (most important):
        If urgency is "emergency" OR the status is any failure/blocked type,
        the recommendation goes straight to the Maintenance Manager.
        Critical and abnormal cases always need senior sign-off.

    Rule 2 — action-level lookup:
        Otherwise, find the role in personas["by_action"] using the action name.

    Rule 3 — default fallback:
        If the action isn't in the mapping, use the "default" persona.
    """
    personas = load_personas()

    if urgency == "emergency" or recommendation_status in _ESCALATION_STATUSES:
        esc = personas.get("escalation", {})
        return esc.get("role", "Maintenance Manager"), esc.get("id")

    by_action = personas.get("by_action", {})
    entry = by_action.get(action_name)
    if entry:
        return entry.get("role", "Maintenance Supervisor"), entry.get("id")

    default = personas.get("default", {})
    return default.get("role", "Maintenance Supervisor"), default.get("id")
...
def _build_required_parts(winner: dict, diagnosis: FaultDiagnosis) -> list:
    ...
    """
    action_type = _get_action_type(winner.get("action_name", ""))
    if action_type is None:
        return []
...
def assemble_recommendation(ranked: list, diagnosis: FaultDiagnosis, risk: RiskAssessment,
    ...
    # --- rationale ---
    action_type_for_rationale = _get_action_type(winner.get("action_name", ""))
    ...
    evidence: dict[str, str] = {
        ...
        "urgency":            (
            f"Computed: 1 - (RUL {risk.rul_min_days}d / fault stage-1 lifespan); "
            f"resolved timing = {timing}"
        ),
    }
...
def recommend_action(diagnosis: FaultDiagnosis, risk: RiskAssessment, guidance: KnowledgeGuidance) -> MaintenanceRecommendation:
    ...
    if base_action_name:
        ...
        if block:
            return _make_blocked_no_part_rec(base_action_name, block, diagnosis, risk)
...
def _get_action_type(action_name: str) -> str | None:
    """
    Map a candidate's action_name to the action_type expected by check_part_for_action.
    Returns None if the action needs no specific part (no inventory check required).
    """
    name = action_name.lower()
    # sensor replacement is NOT a bearing job — must be checked before the
    # generic "replace" match below, and we have no sensor parts in inventory,
    # so it needs no part lookup.
    if "sensor" in name:
        return None
    ...
    if "lubric" in name:
        return "lubricate"
    return None  # inspect_and_monitor, preventive_maintenance, post_repair_qa, etc.
