...
def apply_safe_defaults(diagnosis, risk) -> tuple:
    ...
    if risk.rul_max_days is None:
        ...
        defaulted.append(
            f"RUL max was not provided — defaulted to "
            f"{rul_min + 7} days")
    ...
    if risk.failure_probability is None:
        ...
        defaulted.append(
            "failure_probability was not provided — defaulted to 50%")

    diag_out = (diagnosis.model_copy(update=diag_updates)
                if diag_updates else diagnosis)
    risk_out = (risk.model_copy(update=risk_updates)
                if risk_updates else risk)
    return diag_out, risk_out, defaulted


def detect_uncertainty(diagnosis, risk,
                       defaulted_fields: Optional[List[str]] = None
                       ) -> UncertaintyResult:
    flags = list(defaulted_fields or [])   # defaulted fields come first
    ...
    if (risk.rul_min_days is not None and risk.rul_max_days is not None
        ...
        flags.append(
            ...
            f"is imprecise. Urgency may be higher than shown if RUL is "
            f"closer to {risk.rul_min_days} days.")
    ...
    if not evidence:
        flags.append(
            ...
            "without sensor/vibration evidence to corroborate.")

    # 4. Severity/risk inconsistency
    severity_rank = {"monitor": 0, "stage_1": 1, "stage_2": 2, "stage_3": 3}
    ...
    if rsk - sev >= 2:
        flags.append(
            ...
            f"This may indicate a rapidly degrading asset or upstream "
            f"assessment disagreement. Treat urgency as '{risk.risk_level}'.")
